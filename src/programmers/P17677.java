...
public class P17677 {
    ...
    public static int solution(String str1, String str2) {
        ...
        List<String> strArr1 = makeStringArray(string1);
        List<String> strArr2 = makeStringArray(string2);
        ...
        if (strArr1.size() == 0 && strArr2.size() == 0){
            ...
            answer = (int) Math.floor(intersectionSize/unionSize * 65536);
        }
        System.out.println(answer);
        return answer;
    }

    public static List<String> makeStringArray(String param){
        List<String> list = new ArrayList<>();
        int strLength = param.length();

        //영문자로 된 글자 쌍만 유효하고, 기타 공백이나 숫자, 특수 문자가 들어있는 경우는 그 글자 쌍을 버린다.
        // 문자의 길이는 2이상 1000이하이다
        if(strLength >=2 && strLength <=1000){
            for (int i =0; i< strLength; i++){
                if(i+2 <= strLength){
                    String temp = param.substring(i,i+2);

                    boolean match1 = String.valueOf(temp.charAt(0)).matches("[^a-zA-Z]");
                    boolean match2 = String.valueOf(temp.charAt(1)).matches("[^a-zA-Z]");
